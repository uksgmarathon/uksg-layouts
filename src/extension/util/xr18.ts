...
let conn: osc.UDPPort | undefined;
const fadesInAction: { [k: string]: NodeJS.Timeout | null } = {};
const faderValues: { [k: string]: number } = {};

/**
 * Sends a message over the OSC connection to the XR18.
 * @param msg OSC message
 */
function send(msg: osc.OscMessage) {
  if (!config.enabled || !conn) return; // if not enabled, don't send
  conn.send(msg);
 ...
 */
function setFader(address: string, value: number) {
  send({
    address,
...
function fade(address: string, direction: 'up' | 'down') {
 ...
 */
function logHelper(name: string, connection: osc.UDPPort) {
...
if (config.enabled) {
  ...
  xremoteConn.on('message', (message) => {
    ...
    if (message.address.endsWith('/fader')) {
      const args = Array.isArray(message.args) ? message.args[0] as osc.MetaArgument : undefined;
