...
capturePositions.on('change', async (val) => {
  ...
  if (!nodecg.bundleConfig.obs.enabled || !obs.conn.identified) return;
  // Don't run if there's no "Game Layout" values.
  if (!val[gameLayoutKey]) return;

  const positionAndCrop = async (type: 'game' | 'camera', captureName: string, elemName: string) => {
    ...
    try {
      ...
      nodecg.log.warn(
        '[Game Layout] Error configuring scene item on capture positions change (%s - %s):',
        captureName,
        elemName,
        err,
      );
...
sc.runDataActiveRun.on('change', (newVal, oldVal) => {
  ...
  if (layoutInit && newVal) {
    ...
    if (!oldVal || newVal.id !== oldVal?.id) {
      const code = (newVal.customData.layout as string | undefined)?.toLowerCase();
